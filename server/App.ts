...
export default class App {
  ...
  private async connectDb(): Promise<Db> {
    ...
    const connection = await MongoClient.connect(Conf.database, {
  ...
  private bootstrap(): void {
    ...
    this.quizSessionRepository = new QuizSessionRepository(
      this.database,
      "uq_quizSession"
    );
    ...
    StudentAuthenticatorMiddleware.instantiate(
      this.userService,
      this.userSessionService,
      this.quizSessionService,
  ...
  private setupRoutes(): void {
    ...
    this.express.use(
      this.authFilter("/user/login", expressJwt({ secret: Conf.jwt.SECRET }))
    );
    ...
    if (Conf.express && Conf.express.serveStaticContent) {
      ...
      this.express.use(
        "/client",
        express.static(__dirname + Conf.folders.clientFolder)
      );
      this.express.use(
        "/admin",
        express.static(__dirname + Conf.folders.adminFolder)
      );
    }
  ...
  private refreshJWT(
    ...
    const token = jwt.sign(oldToken, Conf.jwt.SECRET, {
      expiresIn: Conf.jwt.TOKEN_LIFESPAN
    });
