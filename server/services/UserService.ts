...
export class UserService extends BaseService<IUser> {
  ...
  constructor(
    ...
    _chatGroupRepo: ChatGroupRepository,
    _quizSessionRepo: QuizSessionRepository,
    _userSessionRepo: UserSessionRepository,
    _responseRepo: ResponseRepository,
    _courseRepo: CourseRepository,
    _criteriaRepo: CriteriaRepository,
    _rubricRepo: RubricRepository
  ) {
    super();
  ...
  public async handleLoginWrapper(request: ILTIData) {
    ...
    if (isAdmin) {
      ...
      const inputString = Object.keys(request).reduce(
        (str, k) =>
          str + `<input type="hidden" name="${k}" value="${request[k]}" />`,
        ``
      );
  ...
  public async handleBackupLogin(
    request: ILTIData
  ): Promise<BackupLoginResponse> {
  ...
  public async handleLogin(request: ILTIData): Promise<LoginResponse> {
    ...
    const { customQuizId, available } = await (async () => {
      try {
        ...
        if (!quizSchedule)
          throw new Error('Invalid quiz id / missing in database');

        const available =
          Date.now() >= quizSchedule!.availableStart!.getTime() &&
          quizSchedule.availableEnd!.getTime() >= Date.now()
            ? true
            : false;

        return {
          customQuizId: quizSchedule && quizSchedule._id!,
          available,
        };
      } catch (e) {
  ...
  public async handleAdminLogin(
    ...
    const maybeCourse = await this.courseRepo.findAll({
      name: identity.course,
    });
  ...
  public async registerIntermediate(
    token: BackupLoginResponse,
    responsePayload: any[]
  ): Promise<{ token: IntermediateLogin; responses: Response[] }> {
    ...
    if (token.type === LoginResponseTypes.BACKUP_LOGIN && responsePayload) {
      ...
      const relevantResponse = responses.find((response) => {
        return (
          response.questionId === (discussionpage as IDiscussionPage).questionId
        );
      })!;
  ...
  public async handlePageRequest(
    quizId: string,
    pageId: string,
    quizSessionId: string,
    groupId: string | null
  ): Promise<QuestionRequestData | null> {
    return UserServiceHelper.getPageBasedOnIdsAndTime(
      quizId,
      pageId,
      quizSessionId,
      groupId,
  ...
  public async handleReconnect(
    ...
    quizSessionId: string,
    groupId: string | null
  ): Promise<QuestionReconnectData> {
    return UserServiceHelper.getQuizBasedOnTime(
      quizId,
      quizSessionId,
      groupId,
      this.quizRepo,
  ...
  public async handleFetch(
    ...
    if (token.type === LoginResponseTypes.ADMIN_LOGIN) {
      ...
      const criterias = await this.criteriaRepo.findAll({
        course: token.courseId,
      });
      ...
      quizSchedule.pages!.forEach((element) => {
        if (
          (element.type === PageType.QUESTION_ANSWER_PAGE ||
            element.type === PageType.DISCUSSION_PAGE) &&
          questionIds.findIndex((id) => {
      ...
      const questions = await UserServiceHelper.RetrieveQuestions(
        this.questionRepo,
        questionIds
      );
      ...
      token.type === LoginResponseTypes.GENERIC_LOGIN ||
  ...
  async isQuizIdActiveForUserCourse(
    courseId: string,
    quizId: string
  ): Promise<boolean> {
    try {
      const quizzes = await this.quizRepo.findAvailableQuizzesInCourse(
        courseId
      );
      const requestedQuizInQuizzes = quizzes.find(
...
class UserServiceHelper {
  ...
  public static async RetrieveUser(
    ...
    identity: IMoocchatIdentityInfo
  ): Promise<IUser> {
   ...
   */
  public static async RetrieveQuizSchedule(
   ...
   */
  public static async RetrieveQuestions(
  ...
  public static async getQuizBasedOnTime(
    quizId: string,
    quizSessionId: string,
    groupId: string | null,
    quizRepo: QuizRepository,
    ...
    if (!quiz || !quizSession) {
      throw Error(
        `Invalid Quiz Id of ${quizId} or Quiz session Id of ${quizSession}`
      );
    }
    ...
    for (let i = 0; i < firstDiscussionIndex; i++) {
      if (runningTime >= now) {
        ...
        runningTime =
          runningTime + Utils.DateTime.minToMs(quiz.pages![i].timeoutInMins!);
    ...
    (quiz.pages || []).forEach((page) => {
      page &&
        typeof page.timeoutInMins === 'string' &&
        (page.timeoutInMins = parseFloat(page.timeoutInMins));
    });
    ...
    return {
      ...
      lastDiscussionIndex:
        lastDiscussionIndex >= 0 ? lastDiscussionIndex : undefined,
    };
  }

  public static async getPageBasedOnIdsAndTime(
    quizId: string,
    pageId: string,
    quizSessionId: string,
    groupId: string | null,
    ...
    if (
      desiredPage.type === PageType.QUESTION_ANSWER_PAGE ||
      desiredPage.type === PageType.DISCUSSION_PAGE
    ) {
    ...
    if (
      desiredPageIndex > firstDiscussionIndex &&
      firstDiscussionIndex !== -1
    ) {
      requireGroupId = true;
    }
    ...
    if (requireGroupId && groupId) {
      ...
      const timeNeeded = quiz
        .pages!.slice(firstDiscussionIndex, desiredPageIndex)
        .reduce((time, page) => {
      ...
      const timeNeeded = quiz
        .pages!.slice(0, desiredPageIndex)
        .reduce((time, page) => {
          time = time + Utils.DateTime.minToMs(page.timeoutInMins);
          return time;
        }, 0);
