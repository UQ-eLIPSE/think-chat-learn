<template>
	<v-container>
		<v-form ref="form">
			...
			<h3 v-if="isCloning">
				Values have been pre-filled using the data from quiz session "{{ clonedQuizName }}". You can change
				these values as desired for the new quiz session (E.g. Start and end time, quiz title)
			</h3>
				...
				<v-layout row wrap>
							...
							<div class="select-field">
								<select v-model="rubricId">
									<template v-for="rubric in rubricDropDown">
										<option :key="`rubric-${rubric.value}`" :value="rubric.value">{{
											rubric.text
										}}</option>
									</template>
								</select>
						...
						>
							<template v-slot:activator="{ on }">
								<div class="form-control date-field">
									<Validator
										:validationRule="[existenceRule, validDateRule]"
						...
						<div class="form-control">
							<v-layout row class="align-center my-2">
								...
								<span class="checkbox-label ml-2"
									>Public? (If checked, this quiz will be displayed to students)</span
								>
							</v-layout>
						</div>
					</v-flex>
						...
						<v-layout row class="align-start mt-3">
							<div class="page-elevation-controls mr-1">
								...
								<button
									...
									:class="
										`squircular-icon ${
											index === pages.length - 1 ? 'move-btn-disabled' : 'move-btn'
										}`
									"
								>
							...
							<v-flex class="position-relative" xs11>
								<div
									:class="
										`${
											forceValidation && !pageValidation(page)
									...
									>
										<v-layout row wrap class="mt-3 pa-3">
												...
												<div class="select-field">
													<select v-model="page.type">
														<template v-for="pageType in pageTypeDropDown">
															<option
																:key="`pt-${pageType.value}`"
																:value="pageType.value"
																>{{ pageType.text }}</option
															>
														</template>
													</select>
												</div>
											...
											<v-flex xs3 class="form-control">
												<Validator
													:validationRule="[existenceRule, timeoutRule]"
													:value="page.timeoutInMins"
													:forceShowValidation="forceValidation"
												>
												...
												<div
													v-if="
														page.type === PageType.QUESTION_ANSWER_PAGE ||
															page.type === PageType.DISCUSSION_PAGE
													"
												>
													<Validator
														:validationRule="
															page.type === PageType.QUESTION_ANSWER_PAGE
																...
																: [
																		existenceRule,
																		duplicateDiscussionPageRule,
																		checkQuestionPageBeforeDiscussion,
																  ]
														"
														:value="page.questionId"
														:forceShowValidation="forceValidation"
													>
														<span class="input-label required-input"
															>Associate question for the page</span
														>
														<div class="select-field">
															<select v-model="page.questionId">
																<template v-for="question in questionDropDown">
																	<option
																		:key="`pt-${question.value}`"
											...
											>
												<v-layout row class="align-center mt-3 py-2">
													...
													<span class="checkbox-label ml-2"
														>Display responses from question</span
													>
												</v-layout>
											</v-flex>
										</v-layout>
                    ...
                    Adding the `key` resolves the component mounting issue which caused TinyMCE to break if the page order was changed. -->
								...
								<div class="page-multipliable-controls">
									<button
										type="button"
										class="button-cs dark-grey-cl delete-btn compact-btn"
									...
									<button
										type="button"
										class="button-cs green-cl add-btn compact-btn"
										@click="createPage(index + 1)"
									>
					...
					<v-flex xs12>
						<v-container class="controls">
							<button type="button" class="primary-cl button-cs" @click="submitQuiz()">
								{{ pageAction }}
							</button>
						</v-container>
					</v-flex>
...
export default class QuizPage extends Vue {
	...
	private async submitQuiz() {
		...
		const allFieldsFilled =
			nonEmptyFieldArray.every(
				(field) => this.existenceRule(field) && typeof this.existenceRule(field) === "boolean"
			) &&
			this.pages.every(
				(page) =>
					[page.type, page.title, page.timeoutInMins].every(
						(field) => this.existenceRule(field) && typeof this.existenceRule(field) === "boolean"
					) &&
					...
					((page.type !== PageType.QUESTION_ANSWER_PAGE && page.type !== PageType.DISCUSSION_PAGE) ||
						(this.existenceRule(page["questionId"]) &&
							typeof this.existenceRule(page["questionId"]) === "boolean"))
			);
		...
		const validDateTime =
			startDateValidate &&
			typeof startDateValidate === "boolean" &&
			endDateValidate &&
			typeof endDateValidate === "boolean";
		...
		const quizContentValid = this.pages.every((page) => {
			switch (page.type) {
				case PageType.QUESTION_ANSWER_PAGE:
					...
					return (
						this.duplicateQuestionPageRule(page.questionId) &&
						typeof this.duplicateQuestionPageRule(page.questionId) === "boolean"
					);
				case PageType.DISCUSSION_PAGE:
					return (
						this.duplicateDiscussionPageRule(page.questionId) &&
		...
		if (confirmed) {
			...
			const editorResponses = await Promise.all(
				this.pages.map(async (page) => {
					const editorResponse = await saveTinyMceEditorContent(`tmce-${page.__mountedId}`);
			...
			const editorsValid = editorResponses.every((editorResponse) => editorResponse);
			if (editorsValid) {
	...
	pageValidation(page: Page) {
		const allFieldFilled = [page.type, page.title, page.timeoutInMins].every(
			(field) => this.existenceRule(field) && typeof this.existenceRule(field) === "boolean"
		);
		...
		//Check timeout value
		if (!this.timeoutRule(page.timeoutInMins) || typeof this.timeoutRule(page.timeoutInMins) !== "boolean")
			return false;

		if (page.type !== PageType.QUESTION_ANSWER_PAGE && page.type !== PageType.DISCUSSION_PAGE) return true;

		//Check question ID
		if (!this.existenceRule(page.questionId) || typeof this.existenceRule(page.questionId) !== "boolean")
			return false;
		...
		validateQuestionObject.$props.validationRule =
			page.type === PageType.QUESTION_ANSWER_PAGE
